...
def matcher_v1(topic, word, previous_word, terms_match_result):
    ...
    word = str(word).lower()
    phrase = str(FAIR.Language.combine_words(previous_word, word)).lower()
    # -> Loop Weighted Terms
...
def matcher_v2(topic, *content):
    ...
    word_list = FAIR.Language.complete_tokenization_v2(content)
    # -> Get Weighted Terms
    ...
    while processing:
        ...
        for weighted_term in weighted_terms.main_category_keys():
            ...
            expanded_key_list = FAIR.Language.expand_word(weighted_term)
            # -> Loop All Tokens
...
def matcher_v3(*content, topic=None):
    ...
    for topic_name in topic.topic_names:
        ...
        for w_term in weighted_terms:
            # -> Expand Weighted Term
            expanded_key_list = FAIR.Language.expand_word(w_term)
            # -> Loop All Tokens
            for token in word_list:
                if token in expanded_key_list:
